...
export default function QuizGame() {
  ...
  const backgroundClass = useMemo(() => {
    if (gameState === 'start' || gameState === 'end') return 'bg-background';
    return activeTeam === 1 ? 'bg-pink-200' : 'bg-blue-200';
  }, [gameState, activeTeam]);


  return (
    <div className={cn("flex min-h-screen w-full flex-col items-center justify-center p-4 transition-colors duration-1000", backgroundClass)}>
      {gameState === 'start' && (
        ...
        <Card className="w-full max-w-md text-center shadow-2xl border-4 border-primary/50 rounded-2xl animate-in fade-in zoom-in-95">
          ...
          <CardContent className="flex flex-col items-center gap-4 p-8">
            ...
            {scores.team1 !== scores.team2 ? (
               <p className="text-3xl font-bold">{scores.team1 > scores.team2 ? teamNames.team1 : teamNames.team2} venceu!</p>
            ) : (
      ...
      {gameState === 'playing' && (
        <div className="w-full max-w-3xl flex flex-col gap-6 animate-in fade-in">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Card className={cn("transition-all duration-300 shadow-md border-4", activeTeam === 1 ? 'border-pink-400 scale-105' : 'border-transparent', `team-1`)}>
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Users className={cn("h-8 w-8", 'text-pink-400')} />
                    <p className="text-xl font-bold">{teamNames.team1}</p>
                  </div>
                  <p className="text-3xl font-extrabold">{scores.team1}</p>
                </CardContent>
              </Card>
              <Card className={cn("transition-all duration-300 shadow-md border-4", activeTeam === 2 ? 'border-blue-400 scale-105' : 'border-transparent', `team-2`)}>
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Users className={cn("h-8 w-8", 'text-blue-400')} />
